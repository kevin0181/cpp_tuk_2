...
void print_v(Point p[COL][LOW]) {
    ...
    for (int i = 0; i < COL; ++i) {
        ...
        for (int j = 0; j < LOW; ++j) {
            if (p[i][j].shape == "○") { size_w++; size_w_w++; }
            else if (p[i][j].shape == "●") { size_b++; size_w_b++; }
            if (p[j][i].shape == "○") size_h_w++;
            else if (p[j][i].shape == "●") size_h_b++;

            count_continue(i, j, result_w, compare_w, p, size_c_w);
            count_continue(j, i, result_h, compare_h, p, size_c_h);
        ...
        //for (int k = 0; k < size_c_w + 1; ++k) {
          //  cout << "| w" << i << " cnt :" << result_w[k].cnt << " shape:" << result_w[k].shape;
        //}

        for (int k = 0; k < size_c_h + 1; ++k) {
    ...
    for (int i = 0; i < COL * 2; ++i) {
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            if (j < COL && k < LOW) {
                count_continue(j, k, result_slash, compare_slash, p, size_c_slash);
                count_continue(back_slash, k, result_backSlash, compare_backSlash, p, size_c_backSlash);
            }
        }

        for (int k = 0; k < size_c_slash + 1; ++k) {
            cout << " / " << i << " cnt: " << result_slash[k].cnt << " shape: " << result_slash[k].shape;
        }
        for (int k = 0; k < size_c_backSlash + 1; ++k) {
