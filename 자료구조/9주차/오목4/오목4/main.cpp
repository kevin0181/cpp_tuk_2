...
bool undo(Point p[COL][LOW], Point moveHistory[361], int& moveCnt, Point redoHistory[361], int& redoCnt) {
    if (moveCnt <= 0) {
        cout << "무를 수 있는 돌이 없습니다." << endl;
        return false;
    }

    Point lastMove = moveHistory[--moveCnt];

    redoHistory[redoCnt++] = lastMove;
...
bool redo(Point p[COL][LOW], Point redoHistory[361], int& redoCnt) {
    return true;
}
...
int main() {
    ...
    Point moveHistory[361];
    int moveCnt{};
    Point redoHistory[361];
    int redoCnt{};
    ...
    while (true) {
        ...
        if (x == "u") {
            if (undo(p, moveHistory, moveCnt, redoHistory, redoCnt)) {
        ...
        if (!isNumeric(x)) {
            cerr << "문자는 u 또는 r 만 사용할 수 있습니다." << endl;
            continue;
        }
